...
def get_calibrated_threshold(file_path, well_names, control_wells):
    # Placeholder for the calibrated threshold logic
    print("wasap")
    return 0.2
...
def register_plot_callbacks(app):
    @app.callback(
        ...
        [State("well-names-store", "data"),
         State("control-wells-store", "data")],
        prevent_initial_call=True
    )
    def plot_1d_screen(n_clicks, names, control):
        if n_clicks:
            print(f'Control in screen: {control}')
    ...
    def plot_1d(n_clicks_default, n_clicks_calibrated, selected_file, well_names, control_wells):
        ctx = dash.callback_context
        if not ctx.triggered or not selected_file:
        ...
        print(f'Control wells in plot_1d: {control_wells}')
        print(f'Well names in plot_1d: {well_names}')
    ...
    def update_plot_status(n_intervals, plot_status_store):
        if plot_status_store and plot_status_store.get("status") == "processing":
            ...
            if plot_events[file_path].is_set():
                ...
                output_message = html.Div(f"Completed 1D plot with {threshold_type} threshold for probe {probe_name}!", style={"color": "green"})
